...
def pc(grid, md=0, mi=45, fd=0, fi=0, mnd=90, mni=0, fnd=90, fni=0):
    md, mi, fd, fi, mnd, mni, fnd, fni = np.radians([md, mi, fd, fi,
                                                mnd, mni, fnd, fni])
...
def plot1(grid, sources=None, prop='$\Delta T$',
        title='', figname=None, scale=True):
...
def plot23(grid, grid_filt, sources=None, prop='$\Delta T$',
    ...
    plot(grid_filt, fig, axes[1,1], title='Amp. do '+prop+
                ' filtrado\n(domínio do n. de onda)', four=True, log=True)
...
def plot_rtp(grid, md=0, mi=45, fd=0, fi=0, sources=[]):
    grid_filt = rtp(grid, md, mi, fd, fi,)
...
def plot_rte(grid, md=0, mi=45, fd=0, fi=0, sources=[]):
    grid_filt = rte(grid, md, mi, fd, fi,)
...
def plot_pseud(grid, md=0, mi=45, fd=0, fi=0, sources=[]):
    grid_filt = pseud(grid, md, mi, fd, fi,)
...
def plotrtp45():
    grid  = np.loadtxt('45anomaly3d')
    grid += np.loadtxt('45anomaly3d_1515')
    grid += np.loadtxt('45anomaly3d_0015')
    grid += np.loadtxt('45anomaly3d_-20-20')
    grid += np.loadtxt('45anomaly3d_20-20')
...
def plot145():
    ...
    sources = [[50,50],
               ...
               [30,30],
               [30,70]]
    plot1(grid, title='Anomalia causada por várias fontes\n'+
                    '$m_{dec}=0^o, m_{inc}=45^o$', figname='plot45',
                    sources=sources)
...
def plot160():
    ...
    sources = [[30,70]]
    plot1(grid, title='Anomalia causada por várias fontes\n'+
                    '$m_{dec}=0^o, m_{inc}=60^o$', figname='plot60',
                    sources=sources)
